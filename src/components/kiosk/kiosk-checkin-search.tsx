...
import { kioskSearchChildrenAction } from "@/app/actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { AGE_POOL_LABELS } from "@/lib/age";
import type { KioskChildHit } from "@/lib/types";
...
type Props = {
  ...
  onSelect: (child: KioskChildHit) => void;
...
export function KioskCheckInSearch({
  ...
  const [answer, setAnswer] = useState<{ query: string; rows: KioskChildHit[] } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      startTransition(async () => {
        try {
          setAnswer({ query: trimmed, rows: await kioskSearchChildrenAction(trimmed) });
        } catch {
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-black/10 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <p className="truncate text-base text-black/55">
          Parent: {child.parentFirstName} {child.parentLastInitial}
        </p>
        <p className="text-sm text-black/40">
          {child.agePool
            ? AGE_POOL_LABELS[child.agePool]
            : `Age ${child.age} · outside the 4–12 range`}
        </p>
      </div>
      <Button
        size="xl"
        disabled={alreadyIn || !child.agePool || disabled}
        onClick={onSelect}
